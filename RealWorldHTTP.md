...
* 스펙상으로는 쿠키 1개당 4KB
* 근데 브라우저마다 도메인당 최대 쿠키 개수가 다름
* 브라우저 전체 최대 쿠키 개수도 다름
* 그럼 도메인당 최대 쿠키 개수 * 4KB 까지 저장을 허용하느냐고 하면 이것도 브라우저마다 다름
* 왜냐하면 도메인당 쿠키 용량 제한이 있는 브라우저도 있기 때문 (요즘 브라우저들은 제약이 없는 편이나 레거시 브라우저들은 있을 수 있음) 
* http://browsercookielimits.iain.guru/
  * 이에 대해 실험한 사이트
  * 결론은 도메인당 최대 50개, 1개 도메인당 모든 쿠키의 합이 4KB 이하로 하는게 가장 안전
  * 그러나 22년 기준으로 어지간한 브라우저들은 그 이상으로 가능. 다만 애초에 쿠키에 그렇게 많이 저장한다면 설계가 잘못된게 아닐까?
